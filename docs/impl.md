...
- AST node reference (all nodeTypes, subnodes, constants, JSON format): see [php_parser_ast.md](research/php_parser_ast.md).
...
- v5 renamed several nodes (e.g. `LNumber`→`Int_`, `ArrayItem` no longer under `Expr_`). See rename table in [php_parser_ast.md](research/php_parser_ast.md#v5-node-renames).
